...
public class AimAssist extends ClientBase.Module {
    ...
    private static float maximumFovAngle = 105.0F;
    private static double maximumReachBound = 5.0D;
    ...
    private static double maxTurnDeltaPerTick = 24.0D;
    private static boolean stealthProfileMode = true;
    private static int targetAcquisitionDelayTicks = 1;
    ...
    private static boolean biologicalSaccadeActive = true;
    private static int saccadeTickCounter = 0;
    private static boolean vestibularReflexActive = true;
    private static boolean accommodationActive = true;
    private static boolean attentionDriftActive = true;
    private static double peripheralAccuracyLoss = 0.15D;
    private static boolean errorInjectionActive = true;
    private static double randomMissProbability = 0.02D;
    private static boolean memoryRecallActive = true;
    ...
    private static void executeGcdAwareAimPipeline(Minecraft clientRef, Entity target) {
        ...
        if (Math.abs(yawDifference) < minimumBlendThreshold && Math.abs(pitchDifference) < minimumBlendThreshold) {
            return;
        }

        if (errorInjectionActive && secureRandom.nextDouble() < randomMissProbability) {
            yawDifference += (float)((secureRandom.nextDouble() - 0.5) * 2.0D);
            pitchDifference += (float)((secureRandom.nextDouble() - 0.5) * 1.5D);
        }
        ...
        if (windMouseEngineActive) {
            ...
            float curveStepYaw = (float) (yawDifference / 14.0D + cumulativeWindX * 0.010D);
            ...
            if (!horizontalAxisOnly) {
                float curveStepPitch = (float) (pitchDifference / 14.0D + cumulativeWindY * 0.010D);
    ...
    private static void applyAttentionDrift() {
        if (attentionDriftActive && saccadeTickCounter % 30 == 0) {
            kinematicSmoothingRate = 0.14D + (secureRandom.nextDouble() - 0.5) * 0.02D;
        }
    }

    private static void executeAutoCalibrationRoutine() {
        stochasticJitterScale = 0.0008D + secureRandom.nextDouble() * 0.0004D;
        targetPredictionScalar = 1.25D + (secureRandom.nextDouble() - 0.5) * 0.1D;
    }
