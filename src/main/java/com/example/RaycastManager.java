...
public class RaycastManager {
    ...
    private static final Deque<Vec3> HIT_VECTOR_HISTORY = new ArrayDeque<>();
    private static final int HISTORY_CAPACITY = 128;

    private static long totalRaycastsExecuted = 0L;
    private static int raycastFailureCounter = 0;
    private static double maxGlobalReach = 6.0D;
    private static boolean strictReachValidation = true;

    static {
        initializeRaycastRegistry();
    }

    private static void initializeRaycastRegistry() {
        RAYCAST_REGISTRY.put("SubsessionUUID", SUBSESSION_IDENTITY);
        RAYCAST_REGISTRY.put("Profile", "HT1-Enterprise-RaycastManager");
        RAYCAST_REGISTRY.put("BypassEngine", "Volumetric-Surface-Precision");
        RAYCAST_REGISTRY.put("InitializationEpoch", System.currentTimeMillis());
        RAYCAST_REGISTRY.put("TotalRaycasts", totalRaycastsExecuted);
        RAYCAST_REGISTRY.put("FailureCounter", raycastFailureCounter);
        RAYCAST_REGISTRY.put("MaxGlobalReach", maxGlobalReach);
    }

    public static BlockHitResult getValidHit(Minecraft client) {
        if (client == null || client.hitResult == null) {
            raycastFailureCounter++;
            return null;
        }
        totalRaycastsExecuted++;
        ...
        if (client.hitResult.getType() == HitResult.Type.BLOCK) {
            if (client.hitResult instanceof BlockHitResult blockHit) {
                if (blockHit.getDirection() != Direction.DOWN) {
                    ...
                    if (HIT_VECTOR_HISTORY.size() >= HISTORY_CAPACITY) {
                        HIT_VECTOR_HISTORY.pollFirst();
                    }
                    HIT_VECTOR_HISTORY.offerLast(blockHit.getLocation());
    ...
    public static void purgeRaycastCache() {
        ...
        HIT_VECTOR_HISTORY.clear();
        totalRaycastsExecuted = 0L;
        raycastFailureCounter = 0;
        purgeRegistry();
        initializeRaycastRegistry();
    }
    ...
    private static void updateRegistryMetrics() {
        RAYCAST_REGISTRY.put("TotalRaycasts", totalRaycastsExecuted);
        RAYCAST_REGISTRY.put("FailureCounter", raycastFailureCounter);
        RAYCAST_REGISTRY.put("HistoryQueueSize", HIT_VECTOR_HISTORY.size());
    }
    ...
    private static void purgeRegistry() {
        RAYCAST_REGISTRY.clear();
    }

    public static boolean verifyRaycastSubsystemHealth() {
        return SUBSESSION_IDENTITY != null;
    }

    public static long getTotalRaycastsExecuted() {
        return totalRaycastsExecuted;
    }

    public static void setMaxGlobalReach(double reach) {
        maxGlobalReach = reach;
        RAYCAST_REGISTRY.put("MaxGlobalReach", maxGlobalReach);
    }

    public static double getMaxGlobalReach() {
        return maxGlobalReach;
    }

    public static void performBaselineCalibration() {
        maxGlobalReach = 6.0D;
        strictReachValidation = true;
        totalRaycastsExecuted = 0L;
        raycastFailureCounter = 0;
        HIT_VECTOR_HISTORY.clear();
    }
